...
<body>
	<div class="container">
		...
		<form id="myForm" role="form" action="productOptionServlet?method=add&id=${productOptionBean.id}" method="post">
		<!-- 账号密码提交 -->
		<div class="form-group col-md-12">
			<label id="lablel1" class="col-md-2 control-lable" for="name">属性选项名：</label>
				<div class="col-md-10">
					<input class="form-control" type="text" name="name" id="name" placeholder="属性选项名" value="${productOptionBean.name}"/>
				</div>
		</div>
		...
		<div class="form-group col-md-12">
			<label id="label1" class="col-md-2 control-label"  for="name">选项序号：</label>
				<c:if test="${productOptionBean.sort==NULL}">
   					<div class="col-md-10">
					<input class="form-control" type="text" name="sort" id="sort" placeholder="选项序号" value="${productOptionBean.sort}"/>
					</div>    			
   				</c:if>
   				<c:if test="${productOptionBean.sort!=NULL}">
   					<label class="col-md-1 control-label for="username">${productOptionBean.sort}</label>			
   				</c:if>

...
public class ProductTypeServlet extends HttpServlet {
	...
	throws IOException{
		...
		if(id==0){
			...
			if(f){
				resp.sendRedirect("productTypeServlet?method=toAdd&status=true&id=1");
			}else{
				resp.sendRedirect("productTypeServlet?method=toAdd&status=false&id=1");
			}
		}else{
			//修改记录
			...
			if(f){
				resp.sendRedirect("productTypeServlet?method=toAdd&status=true&id=2");
			}else{
				resp.sendRedirect("productTypeServlet?method=toAdd&status=false&id=2");
			}
		}
